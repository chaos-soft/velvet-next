    ...
    <div className='stream' style={code.i ? { backgroundImage: `url(${code.i})` } : {}} ref={stream}>
      <div className={`panel ${chat ? 'hidden' : ''}`}>
        <div className='chats'>
          ...
          {vc && <iframe src={`https://vkplay.live/${code.v}/only-chat?initialMode=0`} />}
        </div>
        <div className='controls'>
          ...
          {code.v && <a className={vc ? 'active' : ''} onClick={() => setVc(!vc)}>vc</a>}
          {code.v && <a className={vp ? 'active' : ''} onClick={() => setVp(!vp)}>vp</a>}
      ...
      {vp && <iframe src={`https://vkplay.live/app/embed/${code.v}`} />}
